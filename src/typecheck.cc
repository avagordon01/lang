...
struct typecheck_fn {
    ...
    ast::type_id operator()(ast::if_statement& if_statement) {
        for (auto& condition: if_statement.conditions) {
            if (std::invoke(*this, condition) != ast::type_id::t_bool) {
    ...
    ast::type_id operator()(ast::for_loop& for_loop) {
        ...
        if (std::invoke(*this, for_loop.condition) != ast::type_id::t_bool) {
    ...
    ast::type_id operator()(ast::while_loop& while_loop) {
        if (std::invoke(*this, while_loop.condition) != ast::type_id::t_bool) {
    ...
    ast::type_id operator()(ast::literal& literal) {
        struct literal_visitor {
            ...
            ast::type_id operator()(bool& x) {
                if (!explicit_type) {
                    *explicit_type = {ast::type_id::t_bool};
                    return *explicit_type;
                }
    ...
    ast::type_id operator()(std::unique_ptr<ast::binary_operator>& binary_operator) {
        ...
        switch (binary_operator->binary_operator) {
            ...
            case ast::binary_operator::C_NE:
                if (l != r) {
                    error(binary_operator->loc, "LHS and RHS of comparison operator are not of the same type. have", ast::type_to_string(l, context.symbols_list), "and", ast::type_to_string(r, context.symbols_list));
                }
                type = {ast::type_id::t_bool};
                break;
